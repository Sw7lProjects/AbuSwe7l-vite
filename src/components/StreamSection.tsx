    ...
    <section className="py-20 bg-gradient-to-br from-[#1e293b] to-[#23242a] relative overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-10">
          <h2 className="text-4xl sm:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-b from-[#60a5fa] to-[#1e40af] mb-4 drop-shadow-lg">Streams & Videos</h2>
          <p className="text-lg text-gray-200 mb-6 drop-shadow">Watch the latest live stream or YouTube video from AbuSwe7l!</p>
        </div>
        <div className="flex justify-center gap-4 mb-8">
          <button
            className={`flex items-center px-6 py-2 rounded-full font-bold text-base transition-all duration-200 border-2 focus:outline-none shadow-glow ${showLive ? 'bg-gradient-to-r from-[#2563eb] to-[#1e40af] text-white border-[#1e40af]' : 'bg-[#23242a]/70 text-white border-[#1e40af] hover:bg-[#2563eb]/10'}`}
              ...
              {showLive ? (
                isLive ? (
                  ...
                  <a href={KICK_CHANNEL_URL} target="_blank" rel="noopener noreferrer" className="w-full h-full block relative">
                    <span className="absolute inset-0 flex items-center justify-center bg-[#23242a]/80 text-3xl font-bold text-gray-light select-none rounded-xl border-2 border-gray-700 shadow-glow-lg">OFFLINE</span>
                  </a>
                )
              ) : (
                <iframe
